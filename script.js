...
function addRow(tableId) {
  ...
  // Get the employee filter based on the table
  ...
  // Get the company filter based on the table
  const companyFilterElement = tableId === 'internalTaskTable' ? document.getElementById("companyFilter") : document.getElementById("companyFilterClient");

  // Collect data from the form (excluding employees)
  ...
  // Log the collected task data from the form
  console.log("Collected form data:", newRowData);

  // Get the selected employees
  const selectedEmployees = Array.from(employeeFilter.selectedOptions).map(option => option.value);
  console.log("Selected Employees:", selectedEmployees); // Log the selected employees

  // Ensure companyFilter is set for internal and client tasks
  const companyFilterValue = companyFilterElement ? companyFilterElement.value : ''; 
  newRowData[tableId === 'internalTaskTable' ? "companyFilter" : "companyFilterClient"] = companyFilterValue; // Set the companyFilter based on table type

  // Log the company filter value
  console.log("Company Filter Value:", companyFilterValue);

  if (selectedEmployees.length > 0) {
    selectedEmployees.forEach(employeeId => {
      console.log(`Fetching data for employee ID: ${employeeId}`); // Log each employee being processed
      
      // جلب بيانات الموظف من Firestore
      db.collection('companies').doc(companyFilterValue).collection('employees').doc(employeeId).get()
        .then(employeeDoc => {
          if (employeeDoc.exists) {
            console.log("Employee Data found:", employeeDoc.data()); // Log the employee data
            
            const employeeData = employeeDoc.data(); // جلب بيانات الموظف بشكل صحيح
            const employeeTaskData = { ...newRowData }; // نسخ بيانات المهمة لكل موظف
            employeeTaskData[tableId === 'internalTaskTable' ? "employeeFilter" : "employeeFilterClient"] = employeeId; // تخصيص معرف الموظف بناءً على الجدول
            employeeTaskData[tableId === 'internalTaskTable' ? "jobTitleFilter" : "jobTitleFilterClient"] = employeeData["المسمى الوظيفي"] || 'غير متاح'; // تخصيص المسمى الوظيفي

            // Log the task data being added for the employee
            console.log("Task Data to be added for employee:", employeeTaskData);

            // إضافة المهمة إلى Firestore لكل موظف
            db.collection(collectionName).add(employeeTaskData)
              .then(() => {
                console.log(`Task successfully added for employee ${employeeId}`); // Log successful task addition
                location.reload(); // Refresh the page after the task is successfully added
              })
              .catch((error) => {
                console.error("Error adding task:", error); // طباعة أي أخطاء تحدث أثناء الإضافة
              });
          } else {
            console.log(`No employee found with ID ${employeeId}`); // Log if the employee does not exist
          }
        })
        .catch((error) => {
          console.error("Error fetching employee data: ", error); // Log error if fetching employee data fails
        });
    });

    // إعادة ضبط النموذج بعد إضافة المهام
    form.reset();
  } else {
    alert("Please select at least one employee");
    console.log("No employees selected"); // Log if no employees are selected
  }
}
...
function addFilteredTaskToTable(task) {
  ...
  orderedKeys.forEach(key => {
    ...
    if (key === 'employeeFilter') {
      ...
      const employeeNames = (task[key] || []).join(", ") || 'اسم غير موجود';
      cell.textContent = employeeNames;
